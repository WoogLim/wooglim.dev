...
Image 태그로 변경한 후에는 logo라는 이미지 객체를 src 프로터피의 값으로 주면 됩니다.
...
`lazy`는 기본 설정된 옵션으로 이미지를 로드해오는대로 화면에 보여주는 방법이고 `srcset`은 다양한 화면 해상도에 맞도록 이미지를 로드하는 설정 방식으로 두 속성 다 Image태그내 기본적으로 설정됩니다.
`width`, `height`의 경우 `import logo from '@/assets/logo.png`로 가져올 동안 자동으로 설정되며 사용자가 이를 오버라이드하여 수정할 수도 있습니다.
