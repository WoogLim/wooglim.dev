...
[참고] 캐시에 관한 데이터는 `.next`폴더에 저장되므로 캐시 테스트를 하고 싶다면 폴더를 제거합니다.
...
전체 라우트 캐시의 효과는 이전 데이터 캐시에서 이미 보고있었습니다.

build를 하게 디면 동적 매개변수가 있는 동적 페이지를 제외하고는 페이지를 미리생성합니다.

다시 캐시 정책을 기본으로 되돌린 후
...
[revalidatePath](https://nextjs.org/docs/app/api-reference/functions/revalidatePath)는 캐시를 제거하는 또 다른 방법중 하나로 `dynamic`, `force-dynamic`, `no-store`의 경우 캐시 자체를 막거나 `expired`옵션을 추가해 유효시간을 설정하는것인 반면
revalidatePath는 특정 캐시 분분을 필요할 때만 재검증 하는 방법입니다. 어느정도 조건부로 적용할 수 있는것이죠.
...
기존 코드에서 `fetch`로 외부 API 요청을 보낸 방법에서 NextJS내 자차 데이터베이스 연결을 통한 방법을 이용하도록 변경합니다.
```js
// messages/layout.js
