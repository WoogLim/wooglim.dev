...
```java
// src/main/java/hello/hello_spring/controller/HelloController.java
// @GetMapping: "/hello-string" 경로로 들어오는 GET 요청을 처리
// @ResponseBody: HTTP 응답 본문에 직접 데이터를 작성하도록 지정
// @RequestParam: HTTP 요청의 쿼리 파라미터 'name'을 메서드 파라미터로 전달
@GetMapping("hello-string")
@ResponseBody
public String helloString(@RequestParam("name") String name) {
    // 문자열을 직접 반환하면 HTTP 응답 본문에 그대로 전달됨
    return "hello " + name;
}
```
