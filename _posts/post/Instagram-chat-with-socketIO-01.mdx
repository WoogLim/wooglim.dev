...
postnumber: "4"
date: "2023-04-27"
thumbnail: "/posts/thumbnail/socket_io_logo.png"
title: "[Socket] Socket.IO를 이용해 인스타그램 채팅 구현하기"
description: 'Socket.Io를 이용해 인스타그램 채팅 구현 하기 - 설계하기'
tag: ["Socket"]
prerequisites: [
    "socket",
    "react",
...
![instagramChat-1](https://github.com/WoogLim/wooglim.dev/assets/51357635/ecd3296b-4f2a-4c0c-a023-d5a6472b561f)
...
![instagramChat-2](https://github.com/WoogLim/wooglim.dev/assets/51357635/dc715690-150f-4249-8712-3247ea08c9b5)

다음은 대상자 선택 후 채팅방이 개설된 모습입니다. 채팅 메시지 및 사진 그리고 이모티콘을 입력할 수 있습니다. 이 부분에서 메시지 전송 기능만을 구현하기로 했습니다.
![instagramChat-3](https://github.com/WoogLim/wooglim.dev/assets/51357635/b02bf419-05ca-45cb-846b-d8c197ecda55)

### 데이터 베이스 설계하기
---
위와 같은 모델을 분석하여 다음과 같이 데이터 베이스를 설계하였습니다. 
![instagramChatERD](https://github.com/WoogLim/wooglim.dev/assets/51357635/02e0ce09-dae3-4d51-a0c8-07ffebc73704)
...
1. `GET [serverURL]/api/chat/` 채팅방 목록 조회하기 - 내가 참여하고 있는 채팅방 목록 조회
2. `GET [serverURL]/api/chat/search` 채팅 대상 검색하기 - 채팅을 시작할 대상자 검색
3. `POST [serverURL]/api/chat/` 채팅방 생성하기 - 채팅 대상자 선택 후 채팅방을 개설. 채팅방에서는 나와 대상자가 참여자로 등록되고 채팅방은 고유의 UUID ID를 부여
4. `GET [serverURL]/api/history` 이전 채팅 목록 불러오기 - 해당 채팅방의 이전 채팅 목록 조회
...
---
이 프로잭트에서는 같은 포트로 express서버와 socket서버를 단일 포트에서 열어두기로 했습니다. 이 방법의 장점은 하나의 포트로 HTTP 서버와 소켓 서버를 실행하면 서버 구성이 단순해지며 클라이언트는 하나의 포트만 사용하여 서버에 연결하면 되어 관리하기 쉬운 장정이 있습니다. 또한 권한 인증 및 프록시 설정(Nginx)및 방화벽 설정이 간편하다는 장점도 존재합니다.  
...
```
위의 코드에서 `app`은 Express 앱을 생성하고 `server`는 http.createServer()를 사용하여 Express 앱을 기반으로 한 HTTP 서버를 생성합니다. 이후 `io`는 Socket.IO 서버를 생성하고 `server`를 이용해 연결합니다. 이와 같은 방법으로 `Express`앱과 `Socket.IO`서버는 같은 포트를 공유하면서 클라이언트와의 연결을 처리할 수 있습니다.
...
io.on("connection", (socket) => {
  ...
  socket.on("joinChatRoom", async () => { // "joinChatRoom" 이벤트가 발생하면 콜백 함수 실행
    socket.join("chatRoom UUID"); // 현재 소켓을 "chatRoom UUID"에 조인
    io.to("chatRoom UUID").emit("hello"); // "chatRoom UUID"에 있는 모든 소켓에게 "hello" 이벤트 emit
  });
});
```
위와 같은 방식으로 1:1 채팅방을 개설할 수 있게됩니다.

다음 장에서는 이어서 백엔드의 세부 로직에 대해 정리해보도록 하겠습니다. 한 주 마무리 잘하시길 바라겠습니다. 감사합니다.
